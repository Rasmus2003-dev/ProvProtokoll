    ...
    try {
      ...
      const res = await fetch('/api/send-protocol', {
        ...
        body: JSON.stringify({
          ...
          subject: `Körprovsresultat – ${state.properties.testType || 'Körprov'} (${state.properties.licenseType || 'B'})`,
          html,
        }),
      });
        ...
        {activeTab === 'email' && (
          <div className="mb-4 border border-gray-200 bg-gray-50 p-4 rounded-xl space-y-3 text-xs text-gray-700 print:hidden" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
            ...
            <div className="flex">
              ...
              <span className="text-black font-bold text-[12px]">Körprovsresultat – {state.properties.testType || 'Körprov'} ({licenseType})</span>
            </div>
